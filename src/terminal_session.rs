...
use crate::{
    activity::{Activity, ActivityTracker, now_ms},
...
impl TerminalSession {
    ...
    pub fn drain(&mut self) -> bool {
        ...
        if let Some(daemon) = &mut self.daemon {
            while taken < DRAIN_BUDGET {
                ...
                taken += bytes.len();
                self.activity.process(&bytes, now_ms());
    ...
    /// What the session is doing, as its terminal reads it: the title it
    /// writes, how often it paints, whether it has hidden the cursor.
    pub fn activity(&self) -> Activity {
        self.activity.report(now_ms())
    }
    ...
    /// Ask the attached runtime to repaint at the current size.  Reattaching
    /// an agent can restore an absolute snapshot that was captured between
    /// two frames of a differential TUI; a real resize is the reliable way to
    /// make it emit a complete frame again, even when the dimensions did not
    /// change.
    pub fn refresh(&mut self) -> Result<()> {
        ...
        if let Some(daemon) = &self.daemon {
            ...
            master
                .resize(PtySize {
                    rows: height,
                    cols: width.saturating_add(1),
                    pixel_width: 0,
                    pixel_height: 0,
                })
                .context("failed to refresh embedded PTY")?;
            master
                .resize(PtySize {
                    rows: height,
                    cols: width,
                ...
                .context("failed to restore embedded PTY size")?;
        }
        debug::log("pty", format!("refreshed at {width}x{height}"));
        Ok(())
    }
