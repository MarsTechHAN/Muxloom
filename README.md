...
| Mouse wheel | Scroll history smoothly in three-line steps |
...
| Wheel over the terminal | Scroll terminal history by three lines |
...
moves by three lines. Muxloom fetches larger bounded chunks, reuses them for
...
`remain-on-exit` keeps an agent pane after its process exits. Discovery maps a
...
`PageUp`、`PageDown` 按可视页移动，滚轮每次移动三行。Muxloom 按 Offset 抓取较大的
...
tmux 的 `remain-on-exit` 会在进程结束后保留 Agent Pane。刷新发现 Codex/Claude
