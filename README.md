...
> New sessions are owned by the bundled `muxloomd` companion, not tmux or a
> hosted agent service. They never appear in `tmux ls`.
...
legacy path only. New launches never create or modify a tmux server.
...
- a compatible `muxloomd` binary installed at the configured
...
Each remote target has exactly one long-lived process:

```text
ssh -T <alias> <companion_command> bridge
```
...
- The agent always runs under detached target-side `muxloomd`, so bridge loss is not
...
legacy backend when tmux exists. All new launches use `muxloomd-*`; no new tmux
session or user tmux metadata is created.
...
- Media streams transfer original encoded bytes/ranges and never remote RGB.
  ...
  not implemented yet.
- Automatic companion bootstrap/upgrade is not implemented yet; place the
  packaged `muxloomd` at each machine's `companion_command` path.
...
新 Launch 不会创建任何 tmux session，因此不会出现在用户的 `tmux ls` 中。
...
- 对应架构的 `muxloomd`，路径由 `companion_command` 配置；
...
- Agent 始终运行在独立的目标 `muxloomd` 下，控制机断线不影响进程生命周期；
...
已经运行的 `ad-*` / `muxloom-*` tmux 会话在目标存在 tmux 时仍走兼容后端；
所有新会话使用 `muxloomd-*`，不会创建新的 tmux Session 或污染用户的 `tmux ls`。
...
- Media Stream 已传原始编码字节/Range，绝不在远端传 RGB；控制端内置音视频解码与
  Pane 内播放 UI 尚未完成；
- companion 自动 Bootstrap/升级尚未完成，需要把 Release 中的 `muxloomd` 放到每台
  机器配置的 `companion_command` 路径；
