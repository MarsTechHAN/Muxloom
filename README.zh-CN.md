...
装到别的机器上时，还会一并询问是否把该 Runtime 的 `sync_files` 送过去——其中包含设置，
以及每个 Agent 存放登录凭证的那个文件——这样远端 Agent 起来后就和本机处在同一环境，而不
是等人再登录一次。这等于把本机的账号交到另一台机器上，所以确认框会把这件事写明，按
`Space` 可以只装不送。装在本机没有东西需要传，因此不会多问这一句。
...
`send_input` 与 `send_input` 类型的 trigger 走同一条链：它们做的是同一件事，只是少了那
层礼节——直接把按键敲进别人的输入框，连"谁在说话"的信封都没有。只在最客气的那扇门上设
限，等于围了一圈栅栏又在旁边留了道门。
...
绝——本地一次，跨机中转借用 controller 凭据之前再一次。每条拒绝都会说清是谁设的限制、
以及绕过它的正确路径，因为读到这条消息的会话自己解不开这个限制。
...
频率比记忆高几千倍——一个聊天账号的租约每几秒就要重新声明一次——而一个只能追加、带序号的 log
没法只丢掉其中一类：中间挖个洞，复制会立刻把它填回来。第二个 log 只保留一千条、十二小时；板子保留
两万条、三十天，且一条都不花在机器自己的流量上。

每条 note 都有 scope，选**它成立的最小那个**：

| Scope | 谁会继承它 | 用途 |
| --- | --- | --- |
| `path`（默认） | 同一台机器同一个目录里的所有人 | 只对某个代码库成立的事 |
...
接手一件事时读一次，遇到反常时搜一次，就够了；它不值得轮询，谁正在做什么请看 `list_sessions`。
需要等的只有 `scope: "direct"`，回信到的地方：`since_cursor` 只返回新增内容，`wait_seconds`
把调用挂住直到有人说话。

一页大到发不回来时会被裁掉，并明说裁了。跟着游标读时保留**最旧**的那一头，游标也只走到那里为止，
所以积压会按顺序、分几次读干净；不带游标读则保留最新的一头，再用 `before` 往回翻。
...
`talk_read { scope: "direct" }`——并且继续用 `message_agent` 回，而不是发到公告板上：
那会把两个 Agent 的对话摆到每台机器上每个 Agent 面前。

人从聊天软件里说话时走的是同一段信封，只是话反过来说：这是人打的字，不是同事的建议。
信封里同样带着回去的路——对方所在频道的 `send_channel_message` 调用，若对方引用了这个
Agent 自己的某条消息，也会带上引用的 id——理由和 Agent 之间那段信封带回信地址一样。
人在手机上不看公告板，回在那里的答复等于没人收到。发到手机上的消息有长度上限——正文 1200
字符、标题 48 字符——超了直接拒绝，而不是截断：截断切掉的永远是 Agent 放在最后的那部分，
通常正是那个「需要你决定什么」，而且 Agent 自己不会知道。拒绝时会说清该改成什么样，这是唯一
能真正让它重写一条短消息的办法。微信还有一种「收下了但没发出去」——同样 HTTP 200、同样
success，只是回包里没有它自己的消息 id——这正是会话 token 过期在这一侧的样子。这种发送会
被明确报成「未送达」而不是「已发送」，不会留下回信要认的 receipt，也会在 dashboard 上记一次
失败。唯一的修法是本人对着 bot 说句话，这边催不动，在那之前重发只会落到同一个地方。

**文件与图片，收发两头，仅飞书。** `send_channel_message { files: [...] }` 收的是 Agent
所在那台机器上的绝对路径，正文发完之后再逐个作为独立消息发出去：图片直接在会话里显示，
其它以文件名下载，图片上限 10 MB、其它 30 MB。有文件传不上去就在正文发出之前整条拒绝，
不会出现「消息看着发全了、附件其实没到」。反过来，人发过来的东西——图片、文件、语音，
或者贴在富文本里的截图——会被下载到读这个聊天的那台机器上，以 0600 存在
`<state>/channel-files/`，文件名做过处理、跑不出这个目录，唤醒 Agent 的那段信封里直接写着
路径，它打开就是了。这些文件放一周后清掉。微信两个方向都只走文字：它的媒体走的是一条
muxloom 不会说的加密 CDN，所以带 files 的发送会被整条拒绝而不是发一半，收到图片时会得到
一行「对方发了张图片，muxloom 取不到」——这至少是 Agent 可以追问的东西，而不是过去那种
一声不响。
...
它的跨机调用由挂着的 Controller 代跑：用 `machine` 参数点名哪台机器，Controller 就在那边
把这次调用跑掉。看和说直接过去——屏幕、文件、会话、历史、对话回忆、`message_agent`、公告板。
在别的机器上"改"什么则先问你：起会话、往会话里打字、归档或删除会话、装触发器、跑 Shell。
这些先回一个 approval id，你在聊天里答复之后 Agent 再调一次即可。若这个「问你」根本没送到
你手上——没绑聊天，或者绑了但那条消息被收下又丢掉——就什么也不会挂起：Agent 会被直接告知
没人被问过，而不是干等一个你从没看见的批准。`launch_session` 点名别的
机器时必须给那边的绝对 `path`——调用方自己的目录在这台机器上。`wait_for` 永远不跨机，它盯的
就是自己所在的机器。机器启停和 SSH 改写则完全不代跑。没有 Dashboard 连着时，所有跨机调用
立刻失败并说明原因。同一台机器可以被不止一个 Controller 盯着，而它们的机队未必相同：调用会
交给能到达它所点名机器的那个 Controller，而不是最先来要活的那个；所有 Controller 都到不了的
机器，会在点名它的那次调用上直接被拒绝并列出它们实际携带的机器，而不是干等中转超时。

Dashboard 借的也是这份可达性。只有别的 Controller 能到的机器，会以 `»` 开头列在你自己的机器
下面，并写着经由哪台过去；选中它就能列出它上面的 Agent，再选一个就能看到它的屏幕，每几秒重取
一次。那是一张静态照片，不是 attach——中继只搬得动一问一答，搬不动一条流。任何会改动那台机器的
操作都不提供：`Enter`、`n`、`x` 会直接说明并告诉你路由是谁。要在那边把事情办掉，请给住在那台
机器上的 Agent 发消息。
...
Agent 退出或第一次按 `x` 后进入 Archived，仍可查看和搜索。打开 Archived 条目就是让它
本人回来：同一个会话 id、同样的 Label、父子关系和子 Agent，历史文件接着写而不是另起一份，
归档里也不会再多出一条旧记录。它重新打开的是记录上写着的那段对话——运行时 daemon 匹配到
的转录，或者从未匹配到时、它自己启动时被告知要接上的那段——所以一个 resume 过、还没人
打字就又被放下的会话，照样知道自己是哪段对话。只有记录上什么都没写、目录里又有好几段时，
才会把选择交给你。此前被 resume 取代过的旧条目（记录上写着对话已迁到列表里另一条）不会
再重复显示。旧式 tmux 会话仍是在归档旁边另起一个，此时确认框会问是否在新 Agent 启动后
移除旧条目，该选择会持久记忆。再次按 `x` 才永久删除 daemon 元数据与历史。
